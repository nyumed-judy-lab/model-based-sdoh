...
load("UNINSUR/VariableSelection-foldsinfo.RData")
load("UNINSUR/VariableSelection.RData")
k=length(M.list)
...
p.uninsur <- ggplot(dat, aes(x=name, y=WCor, group=1)) + geom_point() + geom_line() + 
...
png("fig/UNINSUREstimatedCorrelation.png", units="in", height=6.0*0.6, width=3.8*1.6, res=300)
p.uninsur
dev.off()
...
save(selected.fml, selected.fixed, selected.random, file="UNINSUR/SelectedModelInfo.RData")
