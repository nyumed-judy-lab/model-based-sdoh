...
save(n.folds, folds, dat.sdoh, file=paste0("UNINSUR/VariableSelection-foldsinfo.RData"))
...
for(iter in seq(candidate.fixed)){
  ...
  if(length(idx)==0){ 
    print("!!!!! STOP HERE !!!!!")
    #save.image(paste0("UNINSUR/VariableSelection-iter",iter,".RData"))
    break
...
save(M.list, AIC.list, Fixed, Random, current.fixed, current.random, iter, 
     file="UNINSUR/VariableSelection.RData")
